...
import { InvoiceDoc, computeInvoice } from '@/apps/getpaid/components/InvoiceDoc'
import { useLocalState, encodeShare } from '@/apps/getpaid/lib/storage'
...
export function TradeQuoteApp() {
  ...
  const sendQuote = () => {
    ...
    const url = `${window.location.origin}/tools/one-page-invoice/view#${encodeShare(quote)}`
    const msg = [
  ...
  const convertToInvoice = () => {
    ...
    try {
      const raw = window.localStorage.getItem('verity:getpaid:biller:invoices')
      const list: Invoice[] = raw ? JSON.parse(raw) : []
      window.localStorage.setItem(
        'verity:getpaid:biller:invoices',
        JSON.stringify([...list, invoice]),
      )
      setConverted(true)
    } catch {
